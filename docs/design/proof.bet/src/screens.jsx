...
function Connect({ onConnected }) {
  const [wrongNet, setWrongNet] = useStateSc(true);
  const [connecting, setConnecting] = useStateSc(null);

  const connect = (id) => {
    setConnecting(id);
    setTimeout(() => {setConnecting(null);onConnected(id);}, 900);
  };
    ...
    <div className="center-stage hp-screen">
      <div className="glass connect-card">
        ...
        <div className="net-banner wrong">
            <span>⚠</span>
            <span className="grow">Wrong network — you’re on Ethereum Mainnet.</span>
            <button className="seed-reroll" style={{ color: 'var(--loss)' }} onClick={() => setWrongNet(false)}>Switch to Sepolia</button>
          </div> :
        ...
        <div className="wallet-list">
          {WALLETS.map((w) =>
          <button key={w.id} className="wallet-opt" disabled={wrongNet || connecting} onClick={() => connect(w.id)}>
              <span className="wallet-icon" style={{ background: `linear-gradient(150deg, ${w.tint}, #1a3631)` }}>
                {w.name[0]}
              </span>
              <span>
              ...
              <span className="wallet-go">{connecting === w.id ? 'connecting…' : '→'}</span>
            </button>
          )}
        </div>
