// ============================================================
// HOUSEPROOF — sound engine (Web Audio, synthesized, no assets)
...
(function () {
  ...
  let ctx = null;
  let master = null;
  let climb = null; // { oscs:[{osc,mul}], gain, filt, lfo }
  let muted = (() => { try { return localStorage.getItem(KEY) === '1'; } catch (e) { return false; } })();
  let climbVoice = (() => { try { return localStorage.getItem(VKEY) === 'glass' ? 'glass' : 'velvet'; } catch (e) { return 'velvet'; } })();
  ...
  function voice(freq, t0, dur, { type = 'sine', gain = 0.2, attack = 0.004, glideTo = null, pan = 0 } = {}) {
    const c = ctx;
  ...
  function noise(t0, dur, { gain = 0.15, freq = 1200, q = 0.7, type = 'bandpass' } = {}) {
    const c = ctx;
  ...
  const PK = {
      ...
      if (win) {
        ...
        steps.forEach((s, i) => {
          voice(root * Math.pow(2, s / 12), t + i * 0.06, 0.5, {
            type: 'triangle', gain: 0.14, attack: 0.006,
          });
        });
    ...
    setClimbVoice(v) {
      ...
      try { localStorage.setItem(VKEY, climbVoice); } catch (e) {}
    },
    get climbVoice() { return climbVoice; },

    // start the rising swell — two velvety variants
    limboStart() {
      if (muted || !ensure()) { stopClimb(true); return; }
      stopClimb(true);
      const t = ctx.currentTime;
      ...
      if (climbVoice === 'glass') {
        ...
        filt.frequency.setValueAtTime(900, t); filt.Q.value = 0.4;
        gain.gain.exponentialRampToValueAtTime(0.07, t + 0.45);
        const lfo = ctx.createOscillator();
        const lfoGain = ctx.createGain();
        lfo.type = 'sine'; lfo.frequency.setValueAtTime(4.6, t);
        lfoGain.gain.setValueAtTime(2.5, t);
        lfo.connect(lfoGain);
        const layers = [
          { mul: 1, g: 1.0 }, { mul: 2, g: 0.32 }, { mul: 3, g: 0.12 }, { mul: 0.5, g: 0.4 },
        ];
        ...
        lfo.start(t);
        climb = { oscs, gain, filt, lfo, lfoGain, mode: 'glass', baseFreq: 165, span: 2.3, filtBase: 900, filtSpan: 2200 };
        voice(247, t, 0.45, { type: 'sine', gain: 0.09, glideTo: 494, attack: 0.05 });
      } else {
        // velvet: warm detuned triangle unison + soft sub
        filt.frequency.setValueAtTime(520, t); filt.Q.value = 0.6;
        gain.gain.exponentialRampToValueAtTime(0.075, t + 0.35);
        const layers = [
          { mul: 0.5, detune: 0, type: 'sine', g: 0.55 },
          { mul: 1, detune: -5, type: 'triangle', g: 1.0 },
          { mul: 1, detune: 6, type: 'triangle', g: 0.85 },
          { mul: 1.5, detune: 0, type: 'triangle', g: 0.3 },
        ];
        ...
        filt.connect(gain); gain.connect(master);
        climb = { oscs, gain, filt, mode: 'velvet', baseFreq: 150, span: 2.4, filtBase: 520, filtSpan: 1700 };
    ...
    limboTo(p) {
      ...
      climb.oscs.forEach(({ osc, mul }) => osc.frequency.setTargetAtTime(base * mul, t, climb.mode === 'glass' ? 0.09 : 0.08));
      climb.filt.frequency.setTargetAtTime(climb.filtBase + prog * climb.filtSpan, t, climb.mode === 'glass' ? 0.12 : 0.1);
      // glass: vibrato widens as tension builds
      if (climb.lfoGain) climb.lfoGain.gain.setTargetAtTime(2.5 + prog * 9, t, 0.15);
    },
    // play a short self-contained swell so a tweak change is audible immediately
    limboPreview() {
      if (muted || !ensure()) return;
      this.unlock();
      this.limboStart();
    ...
    limboEnd(win, mult) {
      if (muted) { stopClimb(true); return; }
      if (!ensure()) return;
      const t = ctx.currentTime;
      stopClimb(false);
      if (win) {
        ...
        const steps = big ? [0, 4, 7, 12] : [0, 4, 7];
        steps.forEach((s, i) => voice(root * Math.pow(2, s / 12), t + 0.05 + i * 0.07, 0.5, {
          type: 'triangle', gain: 0.13, attack: 0.006,
        }));
